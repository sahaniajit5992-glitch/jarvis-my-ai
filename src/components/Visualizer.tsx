...
export default function Visualizer({ state, colorOverride, intensityOverride, mode = "circular" }: VisualizerProps) {
  ...
  return (
    <div className="relative flex items-center justify-center w-full aspect-square max-w-[400px]">
      {/* Outer Rotating Ring with Markings */}
      <motion.div
        animate={{ rotate: 360 }}
      ...
      {/* Hexagonal Pattern / Scanning Ring */}
      <motion.div
      ...
      <motion.div
        animate={{ 
          scale: theme.scale,
          boxShadow: `0 0 ${state === 'idle' ? '20px' : '50px'} ${theme.color}44, inset 0 0 20px ${theme.color}44`
        }}
